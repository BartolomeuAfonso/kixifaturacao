            ...
            <div class="col-lg-12" style="padding-left: 50px; padding-right: 50px">
                <div class="card">
                    <div class="card"  style="background:#005c3c;  color:#fff; font-weight: bold;" aria-current="true">
                        <div ><h2>Emissão de Recibo Manual</h2></div>
                    </div>
                    <div class="row">
                        <form method="post" name="form" action="{{ url('salvarRecibo') }}" target="_blank">
                            @csrf
...
<script>
    ...
    function setList(list) {
        ...
        console.log(list);
        var table =
            '<thead id=""class="table table-striped"><tr><td>Designação</td><td>Quantidade</td><td>Preço</td><td>IVA</td><td>Desc.</td><td>Total</td><td></td></tr></thead><tbody>';
        for (var key in list) {
            table += '<tr><td><input style="border-width:0px;border:none;" readonly type="text" value="' +
                list[key].descricao +
    ...
    function getPreco() {
        ...
        if (preco != null && preco > 0 && names != "") {
            ...
            var descricao = names; //nameList[1];
            var ivavalor = prompt("O produto contém IVA? Se sim digite 1 senão 0:");
            var totalIva = 0;
            var iva = 0;
